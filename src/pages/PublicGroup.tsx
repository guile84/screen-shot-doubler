...
import { Loader2, Package, ShoppingBag, Copy, Check, ArrowLeft, LayoutGrid, List } from "lucide-react";
            ...
            <div className="mb-6 flex justify-end">
              <div className="inline-flex rounded-lg border border-border bg-card p-1">
                <button
                  ...
                  className={`inline-flex items-center gap-1.5 rounded-md px-3 py-1.5 text-xs font-medium transition-colors ${view === "grid" ? "bg-primary text-primary-foreground" : "text-muted-foreground hover:text-foreground"}`}
                ...
                <button
                  ...
                  className={`inline-flex items-center gap-1.5 rounded-md px-3 py-1.5 text-xs font-medium transition-colors ${view === "list" ? "bg-primary text-primary-foreground" : "text-muted-foreground hover:text-foreground"}`}
            ...
            {view === "grid" ? (
              ...
              <div className="flex flex-col gap-3">
                {products.map((product) => {
                  const imageUrl = mediaMap?.[product.id];
                  return (
                    <div key={product.id} className="group flex gap-4 overflow-hidden rounded-xl border border-border bg-card p-3 shadow-sm transition-all hover:shadow-md">
                      <div className="h-24 w-24 flex-shrink-0 overflow-hidden rounded-lg bg-muted sm:h-28 sm:w-28">
                        {imageUrl ? (
                          <img src={imageUrl} alt={product.name} className="h-full w-full object-cover transition-transform group-hover:scale-105" loading="lazy" />
                        ) : (
                          <div className="flex h-full w-full items-center justify-center"><Package className="h-8 w-8 text-muted-foreground/40" /></div>
                        )}
                      </div>
                      <div className="flex flex-1 flex-col justify-between gap-2 min-w-0">
                        <div className="space-y-1 min-w-0">
                          ...
                          <h2 className="font-semibold text-foreground line-clamp-1">{product.name}</h2>
                          {product.description && <p className="hidden text-sm text-muted-foreground line-clamp-1 sm:block">{product.description}</p>}
                          {renderPrice(product)}
                        </div>
                      </div>
                      <div className="flex flex-col items-end justify-center gap-2 flex-shrink-0">
                        {product.coupon_code && (
                          <button onClick={() => copyCoupon(product.id, product.coupon_code!)} className="hidden sm:flex items-center gap-2 rounded-lg border border-dashed border-primary/30 bg-primary/5 px-3 py-1.5 transition-colors hover:bg-primary/10">
                            <div className="text-left">
                              <p className="text-[9px] font-medium uppercase tracking-wider text-muted-foreground leading-none">Cupom</p>
                              <p className="font-mono text-xs font-bold tracking-wider text-foreground">{product.coupon_code}</p>
                            </div>
                            {copiedId === product.id ? <Check className="h-3.5 w-3.5 text-primary" /> : <Copy className="h-3.5 w-3.5 text-muted-foreground" />}
                          </button>
                        )}
                        <a href={`/go/${product.slug}`} target="_blank" rel="noopener noreferrer">
                          <Button size="sm" className="gap-1.5"><ShoppingBag className="h-3.5 w-3.5" /> Ver</Button>
                        </a>
                      </div>
                    </div>
                  );
